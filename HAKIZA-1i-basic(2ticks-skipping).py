...
async def reset_state1():
    ...
    try:
        for symbol in SYMBOL_STATES:
            ...
            state["skip_next_ticks1"] = 0
            state["last_executed_non_reversal_count1"] = -1
            state["pending_trade1"] = None
        GLOBAL_SEQUENCE_ACTIVE = False
        ACTIVE_SYMBOL = None
        ACTIVE_BOT = None
        INACTIVE_SYMBOL = None
...
async def handle_tick1(symbol, tick):
    ...
    try:
        ...
        # Handle tick skipping logic for two consecutive ticks
        if state["skip_next_ticks1"] > 0:
            if state["skip_next_ticks1"] == 2:
                # First skipped tick - place trade here
                print(f"{symbol}: First skipped tick {quote} - trade placement point")
                # Execute pending trade if exists
                if state["pending_trade1"] is not None:
                    trade_type = state["pending_trade1"]["trade_type"]
                    stake_amount = state["pending_trade1"]["stake_amount"]
                    print(f"{symbol}: Executing pending trade - {trade_type} with stake {stake_amount}")
                    await place_trades1(symbol, trade_type, stake_amount)
                    state["pending_trade1"] = None
                # Reset current candle ticks and start new candle with this tick
                state["current_candle_ticks1"] = [quote]
                state["skip_next_ticks1"] = 1 
            elif state["skip_next_ticks1"] == 1:
                # Second skipped tick - store as first tick of next candle
                print(f"{symbol}: Second skipped tick {quote} - stored as first tick of new candle")
                state["current_candle_ticks1"] = [quote]
                state["skip_next_ticks1"] = 0 
            return
        ...
        if len(state["current_candle_ticks1"]) == CONFIG["CANDLE_DURATION1"] + 1:
            ...
            # Set to skip two consecutive ticks after candle completion
            state["skip_next_ticks1"] = 2
...
async def detect_non_reversals1(symbol, last_candle_color1, second_last_candle_color1):
    global GLOBAL_SEQUENCE_ACTIVE, ACTIVE_SYMBOL, ACTIVE_BOT, INACTIVE_BOT
    state = SYMBOL_STATES[symbol]["BOT1"]
    try:
...
async def process_candle1(symbol):
    ...
    try:
        ...
        # Store trade info for execution during first skipped tick
        if (
            ...
            state["pending_trade1"] = {
                "trade_type": state["trade_type1"],
                "stake_amount": state["stake_amount1"]
            }
            state["last_executed_non_reversal_count1"] = state["non_reversal_count1"]
            print(f"Trade prepared for {symbol}: {state['trade_type1']} with stake: {state['stake_amount1']} (non_reversal_count: {state['non_reversal_count1']})")
        else:
            print(f"No trade prepared for {symbol} (trade_type: {state['trade_type1']}, stake: {state['stake_amount1']}, non_reversal_count: {state['non_reversal_count1']})")
...
async def place_trades1(symbol, trade_type1, stake_amount1):
    ...
    try:
        print(f"Placing trade for {symbol}: {trade_type1} with stake {stake_amount1}")
    ...
    try:
        ...
        print(f"Trade successfully placed for {symbol}: {trade_type1} with stake {stake_amount1}")
    except APIError as e:
...
async def reset_state2():
    ...
    try:
        for symbol in SYMBOL_STATES:
            ...
            state["skip_next_ticks2"] = 0
            state["last_executed_reversal_count2"] = -1
            state["pending_trade2"] = None
        GLOBAL_SEQUENCE_ACTIVE = False
        ACTIVE_SYMBOL = None
        ACTIVE_BOT = None
        INACTIVE_SYMBOL = None
...
async def handle_tick2(symbol, tick):
    ...
    try:
        ...
        # Handle tick skipping logic for two consecutive ticks
        if state["skip_next_ticks2"] > 0:
            if state["skip_next_ticks2"] == 2:
                # First skipped tick - place trade here
                print(f"{symbol}: First skipped tick {quote} - trade placement point")
                # Execute pending trade if exists
                if state["pending_trade2"] is not None:
                    trade_type = state["pending_trade2"]["trade_type"]
                    stake_amount = state["pending_trade2"]["stake_amount"]
                    print(f"{symbol}: Executing pending trade - {trade_type} with stake {stake_amount}")
                    await place_trades2(symbol, trade_type, stake_amount)
                    state["pending_trade2"] = None
                # Reset current candle ticks and start new candle with this tick
                state["current_candle_ticks2"] = [quote]
                state["skip_next_ticks2"] = 1  
            elif state["skip_next_ticks2"] == 1:
                # Second skipped tick - store as first tick of next candle
                print(f"{symbol}: Second skipped tick {quote} - stored as first tick of new candle")
                state["current_candle_ticks2"] = [quote]
                state["skip_next_ticks2"] = 0  
...
async def process_candle2(symbol):
    ...
    try:
        ...
        if (
            ...
            state["pending_trade2"] = {
                "trade_type": state["trade_type2"],
                "stake_amount": state["stake_amount2"]
            }
            state["last_executed_reversal_count2"] = state["reversal_count2"]
            print(f"Trade prepared for {symbol}: {state['trade_type2']} with stake: {state['stake_amount2']} (reversal_count: {state['reversal_count2']})")
        else:
            print(f"No trade prepared for {symbol} (trade_type: {state['trade_type2']}, stake: {state['stake_amount2']}, reversal_count: {state['reversal_count2']})")
...
async def place_trades2(symbol, trade_type2, stake_amount2):
    """Place trades based on detected trade type for a symbol."""
    try:
        print(f"Placing trade for {symbol}: {trade_type2} with stake {stake_amount2}")
        proposal = await api.proposal({
            "proposal": 1,
    ...
    try:
        ...
        print(f"Trade successfully placed for {symbol}: {trade_type2} with stake {stake_amount2}")
    except APIError as e:
        print(f"APIError while buying contract for {symbol}: {e}. Skipping trade...")
    except Exception as e:
        print(f"Unexpected error while buying contract for {symbol}: {e}. Skipping trade...")
