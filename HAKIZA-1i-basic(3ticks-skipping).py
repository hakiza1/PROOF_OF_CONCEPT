...
SYMBOL_STATES = {
    symbol: {
        "BOT1": {
            ...
            "pending_trade1": None  # Added to store pending trades
        },
        "BOT2": {
            ...
            "pending_trade2": None  # Added to store pending trades
        }
    }
...
async def handle_tick1(symbol, tick):
    ...
    try:
        ...
        if state["skip_next_ticks1"] > 0:
            if state["skip_next_ticks1"] == 3:  
                print(f"{symbol}: First skipped tick {quote} - to be ingnored")   
                state["current_candle_ticks1"] = [quote]  
                state["skip_next_ticks1"] = 2       
            elif state["skip_next_ticks1"] == 2:
                print(f"{symbol}: Second skipped tick {quote} - trade placement point")
                ...
                print(f"{symbol}: Third skipped tick {quote} - stored as first tick of new candle")
                state["current_candle_ticks1"] = [quote]
                state["skip_next_ticks1"] = 0  
            return
